...
export const EvidenceRepository: React.FC<EvidenceRepositoryProps> = ({ isOpen, onClose, onOpenCase }) => {
    ...
    const IntegrityBadge = ({ status }: { status: string }) => {
        if (status === 'Verified') return (
            <span className="flex items-center gap-1 text-[12px] bg-emerald-500/10 text-emerald-400 px-1.5 py-0.5 rounded border border-emerald-500/20 font-medium">
                <CheckCircle2 size={10} /> Verified
            </span>
        );
        if (status === 'Partial') return (
            <span className="flex items-center gap-1 text-[12px] bg-amber-500/10 text-amber-400 px-1.5 py-0.5 rounded border border-amber-500/20 font-medium">
                <AlertTriangle size={10} /> Partial
            </span>
        );
        return (
            <span className="flex items-center gap-1 text-[12px] bg-red-500/10 text-red-400 px-1.5 py-0.5 rounded border border-red-500/20 font-medium">
        ...
        >
            ...
            <div className="h-[72px] px-6 flex items-center justify-between border-b border-white/5 shrink-0 bg-surface/50 backdrop-blur-md">
                <div className="flex flex-col justify-center">
                    ...
                    <div className="text-[12px] text-white/50 mt-1 flex items-center gap-2">
                ...
                <div className="flex gap-2">
                    <div className="relative flex-1 group">
                        ...
                        <input
                            ...
                            className="w-full h-9 bg-black/20 border border-white/10 rounded-lg pl-9 pr-3 text-[12px] text-white placeholder:text-white/30 focus:border-white/20 focus:bg-white/10 outline-none transition-all shadow-neu-pressed"
                        />
                    </div>
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="relative group">
                            <button className="flex items-center gap-1 text-[12px] text-white/40 hover:text-white transition-colors">
                        ...
                        >
                            <Filter size={12} />
                            <span className="text-[12px] font-medium">Filter {activeFilterCount > 0 && `(${activeFilterCount})`}</span>
            ...
            {isFilterExpanded && (
                <div className="p-4 border-b border-white/5 bg-surface/30 animate-in slide-in-from-top-2 duration-200">
                    <div className="flex justify-between items-center mb-3">
                        <span className="text-[12px] font-bold text-white/40 uppercase tracking-wider">Active Filters</span>
                        <button onClick={clearFilters} className="text-[12px] text-blue-400 hover:text-blue-300">Clear all</button>
                    </div>
                            ...
                            <div className="flex gap-1.5 flex-wrap">
                                {['Active', 'Closed', 'Escalated'].map(opt => (
                                    <button
                                        ...
                                        className={`px-2.5 py-0.5 rounded border text-[12px] transition-colors ${filters.status.includes(opt) ? 'bg-blue-500/20 border-blue-500/40 text-blue-300' : 'bg-white/5 border-transparent text-white/60 hover:bg-white/10'}`}
                                    >
                                        {opt}
                                    </button>
                                ))}
                            ...
                            <div className="flex gap-1.5 flex-wrap">
                                {['Verified', 'Partial', 'Flagged'].map(opt => (
                                    <button
                                        ...
                                        className={`px-2.5 py-0.5 rounded border text-[12px] transition-colors ${filters.integrity.includes(opt) ? 'bg-blue-500/20 border-blue-500/40 text-blue-300' : 'bg-white/5 border-transparent text-white/60 hover:bg-white/10'}`}
                                    >
                                        {opt}
                                    </button>
                                ))}
                            ...
                            <div className="flex gap-1.5 flex-wrap">
                                {['Site A', 'Site B', 'Site C', 'Site D'].map(opt => (
                                    <button
                                        ...
                                        className={`px-2.5 py-0.5 rounded border text-[12px] transition-colors ${filters.site.includes(opt) ? 'bg-blue-500/20 border-blue-500/40 text-blue-300' : 'bg-white/5 border-transparent text-white/60 hover:bg-white/10'}`}
                                    >
                                        {opt}
                                    </button>
                                ))}
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar bg-black/20 p-2 space-y-1">
                {filteredCases.length > 0 ? (
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex justify-between items-start mb-1">
                                        <div className="flex items-center gap-2">
                                            <span className="text-[12px] font-bold text-white tracking-tight">{c.id}</span>
                                            <span className="text-[12px] text-white/80 truncate">{c.type}</span>
                                        </div>
                                    </div>

                                    <div className="flex flex-wrap items-center gap-y-1 gap-x-2 text-[12px] text-white/50">
                                        <span>{c.site}</span>
                                        <span className="text-white/20">•</span>
                                        <span>{c.zone}</span>
                                        <div className="flex items-center gap-2 ml-1">
                                            <span className={`px-1.5 rounded uppercase font-bold tracking-wider text-[8px] border ${c.status === 'Active' ? 'text-blue-400 bg-blue-500/10 border-blue-500/20' : 'text-white/40 bg-white/5 border-white/10'}`}>
                                                {c.status}
                                            </span>
                    ...
                    <div className="flex flex-col items-center justify-center h-[300px] text-white/30">
                        ...
                        <p className="text-[12px]">No cases match your filters.</p>
                ...
                <div className="p-3 border-t border-white/10 bg-surface z-30 animate-in slide-in-from-bottom-2 duration-200">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <button onClick={toggleAllSelection} className="text-[12px] text-white/40 hover:text-white">
                ...
                <div className="absolute inset-0 z-[60] bg-black/60 backdrop-blur-sm flex items-center justify-center p-6 animate-in fade-in duration-200">
                    <div className="bg-surface border border-white/10 rounded-xl w-full max-w-[320px] shadow-2xl bg-gradient-surface">
                        ...
                        <div className="p-4 space-y-4">
                            {/* Name (Auto) */}
                            <div>
                                <label className="text-[12px] font-bold text-white/40 uppercase tracking-wider mb-1.5 block">Package Name</label>
                            ...
                            {/* Toggles */}
                            <div>
                                <label className="text-[12px] font-bold text-white/40 uppercase tracking-wider mb-2 block">Include</label>
