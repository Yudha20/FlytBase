            ...
            <div className="p-4 border-b border-white/5 bg-surface/50 backdrop-blur-sm">
                <div className="relative group">
                    ...
                    <input
                        ...
                        className="w-full h-9 bg-white/5 border border-white/5 rounded-lg pl-9 pr-3 text-[12px] text-white placeholder:text-white/30 focus:border-white/20 focus:bg-white/10 outline-none transition-all"
                    />
                    {searchQuery && (
                        <button
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar p-0">
                {filteredLogs.length > 0 ? (
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <div className="flex flex-col items-end shrink-0 w-[70px] pt-0.5">
                                            <span className="text-[12px] font-mono text-white/60">{formatTime(log.timestamp)}</span>
                                            {relTime && <span className="text-[12px] font-mono text-blue-400 font-medium">{relTime}</span>}
                                        </div>

                                        {/* Right: Content */}
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-baseline gap-2 mb-1">
                                                <span className="text-[12px] font-bold text-white truncate">{log.action}</span>
                                                {log.result !== 'INFO' && (
                                                    <span className={`text-[8px] px-1 rounded uppercase font-bold tracking-wider ${log.result === 'SUCCESS' ? 'text-emerald-400' : 'text-white/50'}`}>
                                                        ({log.result})
                                                    </span>
                                                )}
                                            </div>
                                        ...
                                        <div className="px-4 pb-4 pl-[98px]">
                                            <div className="p-3 bg-black/20 rounded-lg border border-white/5 space-y-3">
                                                <div className="grid grid-cols-2 gap-2">
                                                    <div>
                                                        <div className="text-[12px] uppercase tracking-wider text-white/30 font-bold mb-1">Actor</div>
                                                        <div className="text-[12px] text-white">{log.actor}</div>
                                                    </div>
                                                    {log.incidentId && (
                                                        <div>
                                                            <div className="text-[12px] uppercase tracking-wider text-white/30 font-bold mb-1">Incident ID</div>
                                                ...
                                                <div>
                                                    <div className="text-[12px] uppercase tracking-wider text-white/30 font-bold mb-1">Details</div>
                    ...
                    <div className="flex flex-col items-center justify-center h-[300px] text-white/30">
                        ...
                        <p className="text-[12px]">No activity logs found</p>
                    </div>
                )}
            </div>
        </div>
