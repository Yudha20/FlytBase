    ...
    >
      ...
      <div className="h-[72px] px-6 flex items-center justify-between border-b border-white/5 shrink-0 bg-surface/50 backdrop-blur-md">
        <div className="flex items-center gap-2">
           <span className="text-[15px] font-semibold text-white">Alerts</span>
           <span className="bg-black/30 text-white text-[11px] font-bold px-2 py-0.5 rounded-full shadow-inner">{alerts.length}</span>
        </div>
        <button 
            onClick={onClose}
            ...
            return (
                ...
                >
                    ...
                    <div className="flex items-center justify-between w-full pl-1">
                        <span className={`text-[14px] font-semibold leading-none truncate pr-2 drop-shadow-sm ${isSelected ? 'text-white' : 'text-white/80 group-hover:text-white'}`}>{alert.type}</span>
