    ...
    <div className="flex flex-col gap-4 w-full">
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="relative group w-[180px]">
           ...
           <input 
             ...
             className="w-full h-[32px] bg-black/20 border border-white/5 rounded-lg pl-8 pr-3 text-[13px] text-white placeholder:text-white/30 focus:bg-black/30 focus:border-white/10 focus:outline-none transition-all shadow-neu-pressed"
           />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 w-full">
        {filteredSites.length > 0 ? (
          ...
          <div className="col-span-full py-8 text-center text-white/30 text-[13px]">
  ...
  return (
    <div className="bg-gradient-card border border-white/5 rounded-xl p-4 hover:border-white/10 cursor-pointer transition-all duration-300 group flex flex-col h-full min-h-[140px] shadow-neu-flat hover:-translate-y-1 hover:shadow-neu-hover">
      ...
      <div className="flex justify-between items-start mb-1">
        <h3 className="text-[15px] font-semibold text-white group-hover:text-white transition-colors leading-tight drop-shadow-sm">
            {site.name}
        </h3>
        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border uppercase tracking-wide ${getStatusColor(site.status)}`}>
          {site.status}
        </span>
      </div>
      ...
      <div className="flex flex-col gap-1 mb-auto">
         ...
         <div className="text-[13px] font-medium mt-1 flex items-center gap-1.5">
            {isActive ? (
                <>
      ...
      <div className="flex items-center justify-between pt-3 border-t border-white/5 mt-4">
        <span className="text-[11px] text-white/40 font-medium">Last event: {site.lastEvent}</span>
        ...
        <div className="flex items-center gap-1.5">
           ...
           <span className="text-[11px] text-white/60 font-medium">{site.connectionState}</span>
        </div>
      </div>
    </div>
  );
