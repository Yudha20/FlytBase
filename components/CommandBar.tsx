...
export const CommandBar: React.FC<CommandBarProps> = ({ onCommandSent }) => {
  ...
  const renderSiteList = () => (
    <div className="flex flex-col gap-1 py-1">
        <div className="px-3 py-1.5 text-[10px] font-bold text-white/30 uppercase tracking-wider flex justify-between items-center">
        ...
        {showAllSitesOption && (
            ...
            >
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <div className="text-[13px] font-medium leading-tight">All sites</div>
                        <div className="text-[11px] text-white/40 leading-tight mt-0.5">Global scope</div>
                    </div>
                </div>
        ...
        {filteredSites.length > 0 ? (
                ...
                return (
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <div className={`text-[13px] font-medium leading-tight ${isSelected || isHighlighted ? 'text-blue-400' : 'text-white'}`}>
                                    {site.name}
                                </div>
                                <div className="text-[11px] text-white/50 leading-tight mt-0.5">{site.location}</div>
                            </div>
                        </div>
            ...
            <div className="px-4 py-3 text-[13px] text-white/30 italic text-center">No sites found</div>
        )}
    </div>
  );
        ...
        {[
            ...
            <div key={idx} className="flex gap-4 min-h-[40px] relative">
                ...
                <div className={`flex-1 flex justify-between pb-4 transition-opacity duration-300 ${idx > executionStep ? 'opacity-30' : 'opacity-100'}`}>
                    ...
                    <div>
                         {idx < executionStep && <span className="text-[10px] bg-emerald-500/10 text-emerald-400 px-1.5 py-0.5 rounded font-bold tracking-wider shadow-sm">DONE</span>}
                         {idx === executionStep && <span className="text-[10px] bg-blue-500/10 text-blue-400 px-1.5 py-0.5 rounded font-bold tracking-wider flex items-center gap-1 shadow-glow-blue"><Loader2 size={8} className="animate-spin" /> RUNNING</span>}
                         {idx > executionStep && <span className="text-[10px] bg-white/5 text-white/30 px-1.5 py-0.5 rounded font-bold tracking-wider">QUEUED</span>}
                    </div>
                </div>
            </div>
        ))}
                ...
                <div className="p-2 border-b border-white/5">
                    <div className="flex items-center bg-black/30 rounded-lg px-2 h-8 border border-white/5 focus-within:border-white/20 transition-colors shadow-inner">
                        ...
                        <input
                            ...
                            className="bg-transparent border-none outline-none text-[13px] text-white w-full placeholder:text-white/30 h-full"
                            value={searchQuery}
        ...
        <div className="flex items-center gap-2 px-1 relative h-10 w-full z-20" ref={atMenuRef}>
            {scope && (
                <button 
                    ...
                    className="shrink-0 text-[15px] text-blue-400 font-medium hover:text-blue-300 transition-colors mr-1 cursor-pointer"
                >
                    {scope.name}
                </button>
            )}
            ...
            <div className="relative flex-grow h-full flex items-center group">
                {!inputValue && !isExecuting && !isReviewing && (
                    <div className="absolute inset-0 flex items-center pointer-events-none text-[15px] text-white/30">
                ...
                <input
                    ...
                    className="w-full bg-transparent border-none outline-none text-[15px] text-white h-full relative z-10 disabled:opacity-80 disabled:cursor-not-allowed"
                    autoComplete="off"
                />
      ...
      {isReviewing && (
        <div className="px-4 pb-4 pt-1 flex flex-col gap-0 animate-in fade-in slide-in-from-top-2">
           ...
           <div className="mb-6 pl-1">
              <div className="text-[11px] font-bold text-white/40 uppercase tracking-wider mb-3">Execution Plan</div>
              <div className="space-y-3 border-l border-white/10 pl-4 ml-1">
                 <div className="flex items-center gap-2">
                    ...
                    <span className="text-[13px] text-white/80">Queue sweep sequence</span>
                 </div>
                 <div className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 rounded-full bg-white/40 shadow-glow-blue" />
                    <span className="text-[13px] text-white/80">Assign available drone</span>
                 </div>
                 <div className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 rounded-full bg-white/40 shadow-glow-blue" />
                    <span className="text-[13px] text-white/80">Launch and monitor</span>
                 </div>
              </div>
           </div>
